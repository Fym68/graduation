...
def train_transforms(img_size, ori_h, ori_w):
    ...
    if ori_h < img_size and ori_w < img_size:
        ...
        transforms.append(A.Resize(int(img_size), int(img_size),
                                   interpolation=cv2.INTER_NEAREST))

    # Geometric augmentations (image + mask synchronized)
    transforms += [
    ...
    # Normalize AFTER augmentation so border fills and pixel ops use raw [0,255] values
    # These are the same ImageNet values as pixel_mean/pixel_std in DataLoader

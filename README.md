...
1. 必须有的 2 张（你已确认）
    1. 图 3.1 — 整体框架图
        * 两阶段流程：Stage 1（多模态提示生成 + 微调）→ Stage 2（局部感知偏好对齐 + 监督校准）。建议用 draw.io 或 PPT 画，重点突出数据流和各模块的冻结/可训练状态。
    2. 图 3.2 — 定性对比可视化
        * 从 CSV 中挑 3-4 个典型 case（建议选：一个大病灶 Ours 明显好的、一个小病灶边界清晰的、一个困难 case），每行展示：原图 → GT → nnUNet → FCN → MedSAM → SAM-Med2D → Ours，标注每个的 Dice。你已有所有模型的 mask 文件，拼图即可。
2. 建议增加的：
    1. 图 3.3 — 视觉提示生成流程示意
        * 展示：输入切片 → ResNet-18 分类器 → Grad-CAM 热力图 → 二值化 → 最大连通域 → bbox 提取。
        * 你已经有 output_layer4_07/grad_cam_vis/ 下的 Grad-CAM 可视化图，挑一张典型的拼成流程即可。这张图直接支撑 3.3.2 节"视觉提示生成"的叙述，且制作成本极低。
    2. 图 3.4 — DPO 信号稀释问题可视化（核心贡献图）这是你最重要的技术贡献的直观解释。建议展示：
        * 上排：reference 模型的概率图 → 4 个阈值候选 mask（标注像素差异数量）→ 差异区域 mask（$M_{\text{diff}}$）
        * 下排：对比"全局平均 log_prob"vs"局部感知 log_prob"的梯度信号强度
        * 这张图可以快速生成：取一个测试样本，用 Stage 1 模型跑一次 inference 拿到 logits，做 sigmoid 后分别用 4 个阈值二值化，可视化差异区域。代码量很小（~30 行 matplotlib）。
3. 分类器显著图
    1. 建议纳入图 3.3 中，不需要单独成图。
    2. Grad-CAM 热力图作为视觉提示生成流程的一个中间步骤展示即可，不需要像分类论文那样做大量 CAM 分析。你的重点是分割，分类器只是工具。

## 3.4的图
找到了几个有差异像素的样本。11072744-T2S-08 有 179 个差异像素，很适合可视化。让我用这个样本重新生成图。
...
局部感知信号差：0.383120
信号放大倍数：533×
这组数据直接支撑了你论文 3.3.3 节"标准 DPO的局限与改进"的论述——全局平均下偏好信号几乎为零（0.0007），而局部感知方法将其放大了 500+ 倍。

图的布局是：- 上排 6格：原图 → GT → 概率图 → 最优候选(τ=0.3) → 最差候选(τ=0.6) → 差异区域 M_diff
-下排：放大的边界区域（概率图、y_w、y_l、Ω_diff）+ 信号强度对比柱状图

全部子图已保存到 figures/fig34_subfigures/：上排 6张（全局视角）：

文件	内容
a_input.png	原始输入图像
b_gt.png	原图 + GT 轮廓（绿色）
c_prob_map.png	Sigmoid 概率图（热力图 + colorbar）
d_candidate_best.png	最优候选 y_w（青色）vs GT（绿色虚线）
e_candidate_worst.png	最差候选 y_l（红色）vs GT（绿色虚线）
f_diff_mask.png	差异区域 M_diff（黄色高亮，标注像素数）
下排 4 张（放大边界区域）：

文件	内容
g_zoom_prob.png	放大的概率图
h_zoom_best.png	放大的 y_w 轮廓
i_zoom_worst.png	放大的 y_l 轮廓
j_zoom_diff.png	放大的 Ω_diff
额外：
| k_signal_bar.png | 全局 vs 局部信号强度柱状图（533× 放大） |

你可以用LaTeX 的 subfigure 或 minipage 自由组合排版。核心数据：差异像素仅占 0.19%（123/65536），全局信号 0.000719 vs 局部信号 0.383，放大 533 倍。
